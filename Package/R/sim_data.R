...
#' plot_sim can be used to visualize the output.
#'
...
#' @return a data.table containing the trajectories in long format in 4 columns. "variable" indicates the ID of the trajectory. IDs are named V1, V2, ..., Vn for each level of noise.
#' @export
#'
#' @examples
...
#' @param end numeric, end time of simulations.
#'
#' @details Damping sinusoids are generated with the equation: \eqn{y(t)=A . exp(-L.t) . sin(t)}.
#' Where A represents the initial envelope amplitude and lambda the decay constant. damp_params is of the form: c(A,L)
#'
#' sim_expodecay_lagged_stim simulates trajectories that arise from a "stimulation" (i.e. taking the trajectory to a fixed high-level).
#' Relaxation after stimulation is an exponential decay following equation: \eqn{y(t) = A0 . exp(-t/T)}.
#' Where A0 represents the initial level of the trajectory and T the desintegration rate.
#'
#' @return a data.table containing the trajectories in long format in 3 columns. "variable" indicates the ID of the trajectory.
#' @export
#'
#' @seealso multi_sim
#'
...
#' @param damp_params numeric vector of 2, for dampening parameters of the form c(initial amplitude, decay rate).
#'  Used in sim_phase_shifted_damped and sim_noisy_amplitude_damped. See details.
...
#' @param slope numeric, slope of linear trend, i.e. change of mean value per unit of time.
...
#' @param interval.stim Time interval between stimulations. Used in sim_expodecay_lagged_stim only.
#' @param lambda Desintegration rate. Used in sim_expodecay_lagged_stim only. See details.
