...
An alternative representation is tsne, which gives a better separation of the classes. But this is a pure visualization technique, cannot perform an analysis of the components, and output will look different between each run (probabilistic).
...
* Sample X trajectories for each GF, make sure to sample X/4 trajectories per concentration
* Plot heatmap for each GF
* Look at compression rate of the image raw vs compressed (PNG)
...
```{r cache=TRUE, fig.width=15, fig.height=9}
...
## Complexity of heatmaps

Pick a random sample of 40 trajectories, plot a heatmap, save size of the object.

### Size image

#### With unclipped data

Outlier values might drive the whole heatmap as it has to adjust the whole color scale for one value. This might create instability in the size of the resulting file.

Here with PNG compression.

```{r}
plot.heat.save.figure <- function(data, meas.col, n, directory = "plot_heat_output/", save.table = F, palette = heat.colors , ncolor = 20){
  require(gplots)
  # Got to matrix
  temp <- as.matrix(dcast(data, Condition + Label ~ RealTime, value.var = meas.col))[,-c(1,2)]
  class(temp) <- "numeric"
  if(save.table) labels <-  as.matrix(dcast(data, Condition + Label ~ RealTime, value.var = meas.col))[,2]
  
  # Select N rows from this matrix
  smpl <- sample(1:nrow(temp), n)
  temp <- temp[smpl,]
  if(save.table) labels.smpl <- labels[smpl]
  
  cond <- gsub(pattern = "\\.", "_", as.character(unique(data$Condition)))
  cond <- gsub(pattern = "-", "", cond)
  
  # Plot and save heatmap to PNG
  png(paste0(directory, cond, ".png"))
  heatmap.2(temp, dendrogram='none', Rowv=T, Colv=F, trace='none', key = F, labRow = "", labCol = "", lhei = c(1,1e6), lwid = c(1,1e6), col = palette(ncolor))
  dev.off()
  
  #jpeg(paste0(directory, cond, ".jpg"))
  #heatmap.2(temp, dendrogram='none', Rowv=T, Colv=F, trace='none', key = F, labRow = "", labCol = "", lhei = c(1,1e6), lwid = c(1,1e6), col = heat.colors(ncolor))
  #dev.off()
  if(save.table) write.csv(x = data[Label %in% labels.smpl], file = paste0(directory, cond, ".csv"), row.names = F)
  
  # Get the size of the heatmap image file
  return(get.file.size(paste0(directory, cond, ".png")))
}


get.file.size <- function(file){
  # Standard output is the size of the file, write to a temp file
  command <- paste0("bash -c \"stat --format=\"%s\" ", file, "> temp.out\"")
  system(command)
  # Read the text file and return as numeric
  return(as.numeric(readLines("temp.out")))
}
```

```{r cache=TRUE}
conditions <- unique(Yanni$Condition)
temp <- replicate(30, sapply(conditions, function(x) plot.heat.save.figure(Yanni[Condition==x], "Ratio", n = 20, "../plot_heat_output/", ncolor = 30)))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()

temp <- replicate(30, sapply(conditions, function(x) plot.heat.save.figure(Yanni[Condition==x], "Ratio.norm", n = 20, "../plot_heat_output/", ncolor = 30)))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()

temp <- replicate(30, sapply(conditions, function(x) plot.heat.save.figure(Yanni[Condition==x], "Ratio.norm.smooth", n = 20, "../plot_heat_output/", ncolor = 30)))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()
```
...
```{r cache=TRUE}
# Clip outlier values
Yanni_clipped <- copy(Yanni)
set(Yanni_clipped, i = which(Yanni_clipped[, Ratio] > 1800), j=3L, value = 1100) # Ratio is the 3rd column
set(Yanni_clipped, i = which(Yanni_clipped[, Ratio.norm] > 1.4), j=5L, value = 1.1) # Ratio.norm is the 5th column
set(Yanni_clipped, i = which(Yanni_clipped[, Ratio.norm.smooth] > 1.4), j=6L, value = 1.1) # Ratio.norm.smooth is the 6th column
```

```{r cache=TRUE}
temp <- replicate(30, sapply(conditions, function(x) plot.heat.save.figure(Yanni_clipped[Condition==x], "Ratio", n = 20, "../plot_heat_output/", ncolor = 30)))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()

temp <- replicate(30, sapply(conditions, function(x) plot.heat.save.figure(Yanni_clipped[Condition==x], "Ratio.norm", n = 20, "../plot_heat_output/", ncolor = 30)))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()

temp <- replicate(30, sapply(conditions, function(x) plot.heat.save.figure(Yanni_clipped[Condition==x], "Ratio.norm.smooth", n = 20, "../plot_heat_output/", ncolor = 30)))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()
```
...
### With greyscale images and clipped data

#### With 5 levels
```{r cache=TRUE}
temp <- replicate(30, sapply(conditions, function(x) plot.heat.save.figure(Yanni_clipped[Condition==x], "Ratio", n = 20, "../plot_heat_output/", palette = grey.colors, ncolor = 5)))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()

temp <- replicate(30, sapply(conditions, function(x) plot.heat.save.figure(Yanni_clipped[Condition==x], "Ratio.norm", n = 20, "../plot_heat_output/", palette = grey.colors, ncolor = 5)))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()

temp <- replicate(30, sapply(conditions, function(x) plot.heat.save.figure(Yanni_clipped[Condition==x], "Ratio.norm.smooth", n = 20, palette = grey.colors, "../plot_heat_output/", ncolor = 5)))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()
```
...
```{r cache=TRUE}
temp <- replicate(30, sapply(conditions, function(x) plot.heat.save.figure(Yanni_clipped[Condition==x], "Ratio", n = 20, "../plot_heat_output/", palette = grey.colors, ncolor = 30)))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()

temp <- replicate(30, sapply(conditions, function(x) plot.heat.save.figure(Yanni_clipped[Condition==x], "Ratio.norm", n = 20, "../plot_heat_output/", palette = grey.colors, ncolor = 30)))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()

temp <- replicate(30, sapply(conditions, function(x) plot.heat.save.figure(Yanni_clipped[Condition==x], "Ratio.norm.smooth", n = 20, palette = grey.colors, "../plot_heat_output/", ncolor = 30)))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()
```
...
```{r}
plot.heat.save.figure.tiff <- function(data, meas.col, n, directory = "plot_heat_output/", save.table = F, palette = heat.colors , ncolor = 20, compression){
  require(gplots)
  # Got to matrix
  temp <- as.matrix(dcast(data, Condition + Label ~ RealTime, value.var = meas.col))[,-c(1,2)]
  class(temp) <- "numeric"
  ...
  # Plot and save heatmap to PNG
  tiff(paste0(directory, cond, ".png"), compression = compression)
  heatmap.2(temp, dendrogram='none', Rowv=T, Colv=F, trace='none', key = F, labRow = "", labCol = "", lhei = c(1,1e6), lwid = c(1,1e6), col = palette(ncolor))
  dev.off()
...
```{r cache=TRUE}
temp <- replicate(5, sapply(conditions, function(x) plot.heat.save.figure.tiff(Yanni_clipped[Condition==x], "Ratio", n = 20, "../plot_heat_output/", palette = grey.colors, ncolor = 30, compression = "none")))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()

temp <- replicate(5, sapply(conditions, function(x) plot.heat.save.figure.tiff(Yanni_clipped[Condition==x], "Ratio.norm", n = 20, "../plot_heat_output/", palette = grey.colors, ncolor = 30, compression = "none")))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()

temp <- replicate(5, sapply(conditions, function(x) plot.heat.save.figure.tiff(Yanni_clipped[Condition==x], "Ratio.norm.smooth", n = 20, palette = grey.colors, "../plot_heat_output/", ncolor = 30, compression = "none")))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()
```

Just to check that all images have exactly the same size (each pixel is encoded).

#### With run run-length encoding
...
```{r eval=FALSE}
temp <- replicate(1, sapply(conditions, function(x) plot.heat.save.figure.tiff(Yanni_clipped[Condition==x], "Ratio", n = 20, "../plot_heat_output/", palette = grey.colors, ncolor = 30, compression = "rle")))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()
```
...
```{r cache=TRUE}
temp <- replicate(30, sapply(conditions, function(x) plot.heat.save.figure.tiff(Yanni_clipped[Condition==x], "Ratio", n = 20, "../plot_heat_output/", palette = grey.colors, ncolor = 30, compression = "lzw")))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()

temp <- replicate(30, sapply(conditions, function(x) plot.heat.save.figure.tiff(Yanni_clipped[Condition==x], "Ratio.norm", n = 20, "../plot_heat_output/", palette = grey.colors, ncolor = 30, compression = "lzw")))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()

temp <- replicate(30, sapply(conditions, function(x) plot.heat.save.figure.tiff(Yanni_clipped[Condition==x], "Ratio.norm.smooth", n = 20, palette = grey.colors, "../plot_heat_output/", ncolor = 30, compression = "lzw")))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()
```
...
```{r cache=TRUE}
temp <- replicate(30, sapply(conditions, function(x) plot.heat.save.figure.tiff(Yanni_clipped[Condition==x], "Ratio", n = 20, "../plot_heat_output/", palette = grey.colors, ncolor = 30, compression = "lzw+p")))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()

temp <- replicate(30, sapply(conditions, function(x) plot.heat.save.figure.tiff(Yanni_clipped[Condition==x], "Ratio.norm", n = 20, "../plot_heat_output/", palette = grey.colors, ncolor = 30, compression = "lzw+p")))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()

temp <- replicate(30, sapply(conditions, function(x) plot.heat.save.figure.tiff(Yanni_clipped[Condition==x], "Ratio.norm.smooth", n = 20, palette = grey.colors, "../plot_heat_output/", ncolor = 30, compression = "lzw+p")))
rownames(temp) <- conditions
temp2 <- as.data.table(temp, keep.rownames = T)
ggplot(melt(temp2, id.vars = "rn"), aes(x=rn, y =value)) + geom_boxplot()
```

#### With lzw+p and transposed image



### Vertical derivative filtering of the heatmaps


## Coefficient of variation along time

### Mean and standard deviation

```{r}
coeff.var <- Yanni[, .(mean.raw = mean(Ratio),
                       sd.raw = sd(Ratio),
                       mean.norm = mean(Ratio.norm),
                       sd.norm = sd(Ratio.norm),
                       mean.smooth = mean(Ratio.norm.smooth),
                       sd.smooth = sd(Ratio.norm.smooth)),
                       by = c("Condition", "RealTime")]
coeff.var[, c("coeff.raw", "coeff.norm", "coeff.smooth") := list(sd.raw/mean.raw, sd.norm/mean.norm, sd.smooth/mean.smooth)]

ggplot(coeff.var, aes(x = RealTime, y = coeff.raw)) + geom_line() + facet_wrap("Condition") + ggtitle("Coefficient of variation - Raw data")
ggplot(coeff.var, aes(x = RealTime, y = coeff.norm)) + geom_line() + facet_wrap("Condition") + ggtitle("Coefficient of variation - Normalized data")
ggplot(coeff.var, aes(x = RealTime, y = coeff.smooth)) + geom_line() + facet_wrap("Condition") + ggtitle("Coefficient of variation - Normalized and smoothed data")
```

```{r}
coeff.var[, .(sum.raw = sum(coeff.raw, na.rm = T),
              sum.norm = sum(coeff.norm, na.rm = T),
              sum.smooth = sum(coeff.smooth, na.rm = T)),
              by = Condition]
```
...
```{r}
coeff.var.robust <- Yanni[, .(med.raw = median(Ratio),
                       iqr.raw = IQR(Ratio, na.rm = T),
                       med.norm = median(Ratio.norm),
                       iqr.norm = IQR(Ratio.norm, na.rm = T),
                       med.smooth = median(Ratio.norm.smooth),
                       iqr.smooth = IQR(Ratio.norm.smooth, na.rm = T)),
                       by = c("Condition", "RealTime")]
coeff.var.robust[, c("coeff.raw", "coeff.norm", "coeff.smooth") := list(iqr.raw/med.raw, iqr.norm/med.norm, iqr.smooth/med.smooth)]

ggplot(coeff.var.robust, aes(x = RealTime, y = coeff.raw)) + geom_line() + facet_wrap("Condition") + ggtitle("Robust Coefficient of variation - Raw data")
ggplot(coeff.var.robust, aes(x = RealTime, y = coeff.norm)) + geom_line() + facet_wrap("Condition") + ggtitle("Robust Coefficient of variation - Normalized data")
ggplot(coeff.var.robust, aes(x = RealTime, y = coeff.smooth)) + geom_line() + facet_wrap("Condition") + ggtitle("Robust Coefficient of variation - Normalized and smoothed data")
```

```{r}
coeff.var.robust[, .(sum.raw = sum(coeff.raw, na.rm = T),
              sum.norm = sum(coeff.norm, na.rm = T),
              sum.smooth = sum(coeff.smooth, na.rm = T)),
              by = Condition]
```
